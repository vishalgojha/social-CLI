...
function registerAppCommands(program: any) {
  ...
  app
    ...
    .action(async (options: AppInfoOptions) => {
      ...
      let appId = id;
      if (!appId) {
        const credentials = config.getAppCredentials() as AppCredentials;
      ...
      try {
        ...
        if (data.namespace) console.log(chalk.cyan('Namespace:'), data.namespace);
        if (data.category) console.log(chalk.cyan('Category:'), data.category);
        if (data.link) console.log(chalk.cyan('Link:'), data.link);
