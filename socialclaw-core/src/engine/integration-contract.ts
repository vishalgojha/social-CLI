...
export interface EmailContractInput {
  hasApiKey: boolean;
...
export function evaluateEmailContract(input: EmailContractInput) {
  const connected = input.hasApiKey;
  const verified = input.hasFromEmail;
  const ts = Date.parse(String(input.latestLiveVerificationAt || ''));
  const maxAgeMs = Math.max(1, Number(input.maxAgeDays || 30)) * 24 * 60 * 60 * 1000;
  const stale = Number.isFinite(ts) ? (Date.now() - ts > maxAgeMs) : true;
  const testSendPassed = Boolean(input.latestLiveVerificationOk) && !stale;
...
export interface CrmContractInput {
...
export function evaluateCrmContract(input: CrmContractInput) {
  const connected = input.hasEndpointUrl;
  const verified = input.hasEndpointUrl;
  const ts = Date.parse(String(input.latestLiveVerificationAt || ''));
  const maxAgeMs = Math.max(1, Number(input.maxAgeDays || 30)) * 24 * 60 * 60 * 1000;
  const stale = Number.isFinite(ts) ? (Date.now() - ts > maxAgeMs) : true;
  const testSendPassed = Boolean(input.latestLiveVerificationOk) && !stale;
...
export function buildEmailFixSuggestions(input: {
  connected: boolean;
  verified: boolean;
  testSendPassed: boolean;
  stale: boolean;
  ...
  if (!input.connected) {
    out.push({
      id: 'connect_sendgrid_api_key',
      title: 'Connect SendGrid API key',
      action: 'POST /v1/clients/:clientId/credentials/email/sendgrid with apiKey'
    });
  }
  if (!input.verified) {
    out.push({
      id: 'set_sendgrid_from_email',
      title: 'Set from email',
      action: 'POST /v1/clients/:clientId/credentials/email/sendgrid with fromEmail'
    });
  }
  if (!input.testSendPassed) {
    if (!input.liveAllowed) {
      out.push({
        id: 'enable_live_verify',
        title: 'Enable live verify mode',
      ...
      out.push({
        id: 'retry_live_verify',
        title: 'Retry live test-send',
        action: 'POST /v1/clients/:clientId/credentials/email/verify { mode: "live" }'
      });
    } else {
      out.push({
        id: 'run_live_verify',
        title: 'Run live email verification',
        action: 'POST /v1/clients/:clientId/credentials/email/verify { mode: "live" }'
      });
    }
  }
...
export function buildCrmFixSuggestions(input: {
  connected: boolean;
  testSendPassed: boolean;
  stale: boolean;
  liveAllowed: boolean;
  ...
  if (!input.connected) {
    out.push({
      id: 'connect_crm_webhook',
      title: 'Connect CRM webhook endpoint',
      action: 'POST /v1/clients/:clientId/credentials/crm/webhook with endpointUrl'
    });
  }
  if (!input.testSendPassed) {
    if (!input.liveAllowed) {
      ...
      out.push({
        id: 'retry_live_verify',
        title: 'Retry CRM live verification',
        action: 'POST /v1/clients/:clientId/credentials/crm/verify { mode: "live" }'
      });
    } else {
      out.push({
        id: 'run_live_verify',
        title: 'Run CRM live verification',
        action: 'POST /v1/clients/:clientId/credentials/crm/verify { mode: "live" }'
      });
    }
  }
