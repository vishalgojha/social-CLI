...
function browserDriverMissingError(cause = null, details = {}) {
    ...
    if (cause)
        error.cause = cause;
    if (details.stdout)
        error.stdout = String(details.stdout);
    if (details.stderr)
        error.stderr = String(details.stderr);
    return error;
}
...
function loadPlaywrightPackage(options = {}) {
    if (PLAYWRIGHT_INSTANCE && !options.forceReload)
...
async function loadPlaywrightOrThrow(options = {}) {
    ...
    if (chromiumExecutableExists(playwright, existsSyncImpl))
        return playwright;
    ...
    if (chromiumExecutableExists(reloaded, existsSyncImpl))
        return reloaded;
    throw browserDriverMissingError(null, { reason: 'Chromium executable is not installed' });
}
