  ...
  async handleApi(req, res, parsedUrl) {
    ...
    if (req.method === 'GET' && route === '/api/team/status') {
      try {
        ...
        const operator = typeof config.getOperator === 'function'
          ? config.getOperator()
          : { id: '', name: '' };
        const actorId = operator.id || rbac.currentUser();
    ...
    if (req.method === 'POST' && route === '/api/team/role') {
      try {
        ...
        const role = rbac.normalizeRole(body.role || '');
        const workspace = body.workspace || config.getActiveProfile() || 'default';
        if (!user) {
          sendJson(res, 400, { ok: false, error: 'Missing user.' });
          return;
        }
        const current = typeof config.getOperator === 'function'
          ? config.getOperator()
          : { id: '' };
        const actor = current.id || rbac.currentUser();
        rbac.assertCan({ workspace, action: 'admin', user: actor });
