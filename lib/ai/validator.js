...
function addMissingSuggestions(field, out) {
  if (field === 'page') {
    ...
    out.push("Missing ad account id. Provide one like 'act_123456789'.");
  } else if (field === 'name') {
...
async function validateIntent(intent, config) {
  ...
  if (!intentsSchema[intent.action]) {
  ...
  const required = toArray(intentsSchema[intent.action]?.required);
