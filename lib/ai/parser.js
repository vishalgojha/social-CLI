...
function logDebug(debug, message, payload) {
  ...
  if (payload === undefined) {
    console.log(`[meta-ai] ${message}`);
    return;
  }
  console.log(`[meta-ai] ${message}`, payload);
}
...
async function aiParseIntent(text, cfg = {}) {
  ...
  const model = process.env.META_AI_MODEL || 'gpt-4o-mini';
  const apiKey = process.env.OPENAI_API_KEY || process.env.META_AI_KEY || '';
