...
async function aiParseIntent(text, cfg = {}) {
  ...
  const model = process.env.SOCIAL_AI_MODEL || process.env.META_AI_MODEL || 'gpt-4o-mini';
  const apiKey = process.env.OPENAI_API_KEY || process.env.SOCIAL_AI_KEY || process.env.META_AI_KEY || '';
