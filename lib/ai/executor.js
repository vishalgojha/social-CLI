...
async function resolveFacebookPageContext(pageInput, config) {
  ...
  if (!userToken) throw new Error('No Facebook token found. Run: meta auth login -a facebook');
  ...
  if (!raw) throw new Error('Missing target page. Set one with meta post pages --set-default or specify it in the prompt.');
...
async function runPostInstagram(intent, config, counters) {
  ...
  if (!token) throw new Error('No Instagram/Facebook token found. Run: meta auth login -a facebook');

  const igUserId = intent.accountId || (config?.getDefaultIgUserId ? config.getDefaultIgUserId() : '');
  if (!igUserId) {
    throw new Error('Missing IG user id. Set default via meta instagram accounts list --set-default.');
  }
...
async function runPostWhatsApp(intent, config, counters) {
  ...
  if (!token) throw new Error('No WhatsApp token found. Run: meta auth login -a whatsapp');
...
async function runQueryPages(intent, config, counters) {
  ...
  if (!token) throw new Error('No Facebook token found. Run: meta auth login -a facebook');
...
async function runQueryMe(intent, config, counters) {
  ...
  if (!token) throw new Error(`No ${api} token found. Run: meta auth login -a ${api}`);
...
async function runQueryInstagramMedia(intent, config, counters) {
  ...
  const igUserId = intent.accountId || (config?.getDefaultIgUserId ? config.getDefaultIgUserId() : '');
  if (!igUserId) {
    throw new Error('Missing IG user id. Set one with meta instagram accounts list --set-default.');
  }
  const client = new MetaAPIClient(token, 'instagram');
...
async function runQueryInsights(intent, config, counters) {
  ...
  if (!act) {
    throw new Error('Missing ad account id. Provide accountId in prompt or set default via meta marketing set-default-account.');
  }
