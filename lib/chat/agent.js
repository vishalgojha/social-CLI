...
function buildClarificationDecision(userInput, reason = 'unknown') {
  const ranked = DEFAULT_CLARIFICATION_CHOICES
    .map((choice) => ({ choice, score: scoreClarificationChoice(userInput, choice) }))
    .sort((a, b) => b.score - a.score)
    .map((row) => row.choice);
  const fallback = DEFAULT_CLARIFICATION_CHOICES.filter(
    (x) => !ranked.some((r) => r.prompt === x.prompt)
  );
...
class AutonomousAgent {
  ...
  async heuristicDecision(userInput) {
    ...
    if (!intent || !intent.action || intent.action === 'unknown_input' || !this.isSupportedTool(intent.action)) {
      const clarification = buildClarificationDecision(userInput, 'unknown');
      return {
        ...clarification,
        suggestions: uniq([
          ...(clarification.suggestions || []),
          'Try: "social hatch" for the new agentic TUI flow'
        ])
      };
    }
    if (shouldAskClarification(userInput, intent)) {
      const clarification = buildClarificationDecision(userInput, 'ambiguous');
      return {
        ...clarification,
        suggestions: uniq([
          ...(clarification.suggestions || []),
          'If you prefer local AI without cloud keys: social agent setup --provider ollama --model llama3.1:8b'
        ])
      };
    }
