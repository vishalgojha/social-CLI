...
function hasLlmKey() {
  return Boolean(process.env.OPENAI_API_KEY || process.env.SOCIAL_AI_KEY || process.env.META_AI_KEY);
}
...
class AutonomousAgent {
  ...
  async process(userInput) {
    ...
    if (decision.actions.length > 0) {
      ...
      if (hasHighRisk(decision.actions, this.devToolByName)) {
  ...
  async tryLlmDecision() {
    try {
      ...
      const model = process.env.SOCIAL_CHAT_MODEL || process.env.META_CHAT_MODEL || process.env.SOCIAL_AI_MODEL || process.env.META_AI_MODEL || 'gpt-4o-mini';
      const key = process.env.OPENAI_API_KEY || process.env.SOCIAL_AI_KEY || process.env.META_AI_KEY;
