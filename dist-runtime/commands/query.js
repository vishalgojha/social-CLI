...
function registerQueryCommands(program) {
    ...
    query
        ...
        try {
            ...
            const rows = asRecordArray(result?.data).map((page) => ({
                id: String(page.id || ''),
            ...
            rows.forEach((page, i) => {
                ...
                if (page.fan_count !== '')
                    console.log(chalk.cyan('   Fans:'), page.fan_count);
            });
            console.log('');
        }
        catch (error) {
    ...
    query
        ...
        try {
            ...
            const rows = asRecordArray(result?.data).map((media) => ({
                id: String(media.id || ''),
    ...
    query
        ...
        try {
            ...
            const rows = asRecordArray(result?.data).map((post) => ({
                id: String(post.id || ''),
            ...
            rows.forEach((post, i) => {
                ...
                if (post.message)
                    console.log(chalk.cyan('   Message:'), post.message);
                if (post.permalink_url)
                    console.log(chalk.cyan('   URL:'), post.permalink_url);
            });
            console.log('');
        }
        catch (error) {
