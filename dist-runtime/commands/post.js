...
function registerPostCommands(program) {
    ...
    post
        ...
        if (hasUrl) {
            ...
            console.log(chalk.cyan('  Page:'), `${pageName} (${pageId})`);
            if (draft)
                console.log(chalk.cyan('  Published:'), chalk.yellow('No (unpublished)'));
            console.log('');
            return;
        }
        ...
        try {
            const response = await axios_1.default.post(`${pageClient.baseUrl}${endpoint}`, form, {
    ...
    post
        ...
        try {
            ...
            if (context)
                console.log(chalk.cyan('  Page:'), `${context.pageName} (${context.pageId})`);
            console.log('');
        }
        catch (error) {
            spinner.stop();
